...
static bool shrink_node(pg_data_t *pgdat, struct scan_control *sc)
{
	...
	do {
		...
		do {
			...
			/*donot do vmpressure when in RSC_ZRAM_MODE_ANON*/
			if (sc->rsc_mode != RSC_ZRAM_MODE_ANON)
#endif
...
#ifdef CONFIG_RSC_ZRAM
		/*donot shrink slab when in RSC_ZRAM_MODE_ANON*/
		if (sc->rsc_mode != RSC_ZRAM_MODE_ANON) {
#endif
		/*
				...
				gfp_t gfp_mask, nodemask_t *nodemask)
{
	...
	if (unlikely(!current_is_kswapd() && (rsc_mem_pri_enable) &&
		is_background(rsc_task_group(current))))
		/*background task do more work*/
		sc.nr_to_reclaim += SWAP_CLUSTER_MAX/4;
#endif
...
#define MEM_4G_BYTES 0x100000000
#define MEM_8G_BYTES 0x200000000
static int __init kswapd_init(void)
{
	int nid, ret;


	if ((totalram_pages << PAGE_SHIFT) < MEM_4G_BYTES) {
		vm_swappiness = 80;
#ifdef CONFIG_RSC_ZRAM_SWAPPINESS
		rsc_zram_swappiness = 100;
#endif
	} else if ((totalram_pages << PAGE_SHIFT) < MEM_8G_BYTES) {
		vm_swappiness = 60;
#ifdef CONFIG_RSC_ZRAM_SWAPPINESS
		rsc_zram_swappiness = 100;
#endif
