...
#include "gc02m1bPD2138mipimono_Sensor.h"
#include "../imgsensor_i2c.h"
/****************************Modify Following Strings for Debug****************************/
#define PFX "GC02M1BPD2138_camera_sensor"
#define LOG_1 LOG_INF("GC02M1BPD2138,MIPI 1LANE\n")
/****************************   Modify end    *******************************************/
...
static struct imgsensor_info_struct imgsensor_info = {
	...
	.sensor_id = GC02M1BPD2138_SENSOR_ID,
...
otp_error_code_t gc02m1bv1_OTP_ERROR_CODE = OTP_ERROR_CODE_NORMAL;
MUINT32  sn_inf_main_gc02m1bv1[13];  /*0 flag   1-12 data*/
MUINT32  material_inf_main_gc02m1bv1[4];  
...
void vivo_read_eeprom(void)
{
	...
	for(;i<=0xF8;i+=8){	
		...
		write_cmos_sensor(0x17, i);	
		write_cmos_sensor(0xf3, 0x34);
		while((read_cmos_sensor(0xf3)&(1<<6))!=0){
			timeout++;
			mdelay(5);
			if(timeout>10)break;
		}	
		value=read_cmos_sensor(0x19);
		LOG_INF("[marklee] read addr[%02x] = 0x%02x %d\n",i,value,timeout);
		timeout=0;
	}
}
...
void vivo_read_eeprom3(void)
{
	...
	for(j=0;i<0xE0;i+=8,j++){	
		...
		value =read_cmos_sensor(0x19);
		sn_inf_main_gc02m1bv1[j]= value;//read_cmos_sensor(0x19);
		LOG_INF("[marklee] read addr[%02x] = 0x%04x\n",i,sn_inf_main_gc02m1bv1[j]);
		timeout=0;
	}
	for(j=0;i<0xFF;i+=8,j++){
		write_cmos_sensor(0x17, i);	
		write_cmos_sensor(0xf3, 0x34);
		while((read_cmos_sensor(0xf3)&(1<<6))!=0){
			timeout++;
		...
		value =read_cmos_sensor(0x19);
		material_inf_main_gc02m1bv1[j]=value;//read_cmos_sensor(0x19);
		LOG_INF("[marklee] read addr[%02x] = 0x%04x\n",i,material_inf_main_gc02m1bv1[j]);
		timeout=0;
	}
}
				  ...
				  UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_CUSTOM_INFO:
		printk("SENSOR_FEATURE_GET_CUSTOM_INFO information type:%lld  gc02m1bv1_OTP_ERROR_CODE:%d \n", *feature_data,gc02m1bv1_OTP_ERROR_CODE);
		switch (*feature_data) {
			case 0:    //info type: otp state
			printk("*feature_para_len = %d, sizeof(MUINT32)*13 + 2 =%ld, %d\n", *feature_para_len, sizeof(MUINT32)*13 + 4,sizeof(unsigned long long));
			if (*feature_para_len >= sizeof(MUINT32)*13 + 4) {
				*(MUINT32 *)(uintptr_t)(*(feature_data+1)) = gc02m1bv1_OTP_ERROR_CODE;//otp_state
				memcpy( feature_data+2, sn_inf_main_gc02m1bv1, sizeof(MUINT32)*13); 
				memcpy( feature_data+10, material_inf_main_gc02m1bv1, sizeof(MUINT32)*4); 
			}
				break;
			}
			break;
...
UINT32 GC02M1BPD2138_MIPI_MONO_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
