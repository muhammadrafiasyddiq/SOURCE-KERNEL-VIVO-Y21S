...
#ifdef CONFIG_VIVO_LCM_REDUCE_MIPI_DRIVER
		if (lcm_software_id == 0x14) {
		printk("VIVO reduce mipi driver for gps jamming\n");
			mdelay(1);
			MIPITX_OUTREGBIT(DSI_PHY_REG[i]+MIPITX_VOLTAGE_SEL, FLD_RG_DSI_HSTX_LDO_REF_SEL, 0x6);//default 0x8
		}
#endif
		if (data_Rate != 0) {
			unsigned int tmp = 0;
	...
	if (en) {
		if (!strcmp(mtkfb_lcm_name, "pd2066_nt36525b_fhdplus_dsi_vdo_boe")) {
			...
			LCM_INFO("%s,lcm(%s) txd nt36525b mipiclk change to 424 \n",
				__func__, mtkfb_lcm_name);
		} else if (!strcmp(mtkfb_lcm_name, "pd2066_nt36525b_fhdplus_dsi_vdo_inx")) {
			def_data_rate = 424*2;
			def_dsi_hbp = 80*3-10; /* daptive HBP 40, for sync_event mode ,  (HBP+HSA)*3-10  */
			def_dsi_hbp = ALIGN_TO((def_dsi_hbp), 4);
			LCM_INFO("%s,lcm(%s) inx nt36525b mipiclk change to 424 \n",
				__func__, mtkfb_lcm_name);
		...
		} else if (!strcmp(mtkfb_lcm_name, "pd2143_nt36525b_fhdplus_dsi_vdo_boe")) {
			def_data_rate = 424*2;
			def_dsi_hbp = 76*3-10; /* daptive HBP 40, for sync_event mode ,  (HBP+HSA)*3-10  */
			def_dsi_hbp = ALIGN_TO((def_dsi_hbp), 4);
			LCM_INFO("%s,lcm(%s) boe nt36672b mipiclk change to 424 \n",
				__func__, mtkfb_lcm_name);
		} else if (!strcmp(mtkfb_lcm_name, "pd2143_ili9882n_fhdplus_dsi_vdo_tm")) {
			def_data_rate = 424*2;
			def_dsi_hbp = 82*3-10; /* daptive HBP 40, for sync_event mode ,  (HBP+HSA)*3-10  */
			def_dsi_hbp = ALIGN_TO((def_dsi_hbp), 4);
			LCM_INFO("%s,lcm(%s) ili9882n mipiclk change to 424 \n",
				__func__, mtkfb_lcm_name);
		} else if (!strcmp(mtkfb_lcm_name, "pd2143_ft8656_fhdplus_dsi_vdo_hx")) {
			def_data_rate = 424*2;
			def_dsi_hbp = 140*3-10; /* daptive HBP 40, for sync_event mode ,  (HBP+HSA)*3-10  */
			def_dsi_hbp = ALIGN_TO((def_dsi_hbp), 4);
			LCM_INFO("%s,lcm(%s) ft8656 mipiclk change to 424 \n",
				__func__, mtkfb_lcm_name);
		} else if (!strcmp(mtkfb_lcm_name, "pd2143_nt36525b_fhdplus_dsi_vdo_txd")) {
			def_data_rate = 424*2;
			def_dsi_hbp = 76*3-10; /* daptive HBP 40, for sync_event mode ,  (HBP+HSA)*3-10  */
			def_dsi_hbp = ALIGN_TO((def_dsi_hbp), 4);
			LCM_INFO("%s,lcm(%s) txd nt36525b mipiclk change to 424 \n",
				__func__, mtkfb_lcm_name);
		} else if (!strcmp(mtkfb_lcm_name, "pd2138_nt36525b_fhdplus_dsi_vdo_boe")) {
			def_data_rate = 424*2;
			def_dsi_hbp = 76*3-10; /*  daptive HBP 40, for sync_event mode ,  (HBP+HSA)*3-10  */
			def_dsi_hbp = ALIGN_TO((def_dsi_hbp), 4);
			LCM_INFO("%s,lcm(%s) boe nt36525b mipiclk change to 424 \n",
				__func__, mtkfb_lcm_name);
		} else if (!strcmp(mtkfb_lcm_name, "pd2138_icn9911c_fhdplus_dsi_vdo_tm")) {
			def_data_rate = 424*2;
			def_dsi_hbp = 56*3-10; /* daptive HBP 40, for sync_event mode ,  (HBP+HSA)*3-10  */
			def_dsi_hbp = ALIGN_TO((def_dsi_hbp), 4);
			LCM_INFO("%s,lcm(%s) tm icn9911c mipiclk change to 424 \n",
				__func__, mtkfb_lcm_name);
		} else if (!strcmp(mtkfb_lcm_name, "pd2147_hx83112f_fhdplus_video_boe_drv")) {
			def_data_rate = 1097;
			def_dsi_hbp = 16*3-10; /* daptive HBP 40, for sync_event mode ,  (HBP+HSA)*3-10  */
			def_dsi_hbp = ALIGN_TO((def_dsi_hbp), 4);
			LCM_INFO("%s,lcm(%s) boe hx83112f mipiclk change to 548.5 \n",
				__func__, mtkfb_lcm_name);
		} else if (!strcmp(mtkfb_lcm_name, "pd2147_ft8720_fhdplus_hx_drv")) {
			def_data_rate = 1097;
			def_dsi_hbp = 26*3-10; /* daptive HBP 40, for sync_event mode ,  (HBP+HSA)*3-10  */
			def_dsi_hbp = ALIGN_TO((def_dsi_hbp), 4);
			LCM_INFO("%s,lcm(%s) hx ft8720 mipiclk change to 548.5 \n",
				__func__, mtkfb_lcm_name);
		} else if (!strcmp(mtkfb_lcm_name, "pd2147_tm658_nt36672c_fhdplus_drv")) {
			def_data_rate = 1097;
			def_dsi_hbp = 20*3-10; /* daptive HBP 40, for sync_event mode ,  (HBP+HSA)*3-10  */
			def_dsi_hbp = ALIGN_TO((def_dsi_hbp), 4);
			LCM_INFO("%s,lcm(%s) tm nt36672c mipiclk change to 548.5 \n",
				__func__, mtkfb_lcm_name);
		} else {
	...
	void *cmdq_trigger_handle, enum CMDQ_STATE state)
{
	...
	("[DSI]para_list[0]=0x%x,para_list[1]=0x%x, para_list[2]=0x%x\n",
			...
			for (j = 0; j < lcm_esd_tb->count; j++) {
				...
				printk("buffer[%d]  read panel status=0x%x\n", j, buffer[j]);
				if (buffer[j] != lcm_esd_tb->para_list[j]) {
				printk("buffer[%d]0x%x != lcm_esd_tb->para_list[%d]0x%x\n",
