...
/*  vivo add start */
#ifdef CONFIG_TUSB422
void vivo_usb_otg_notify_init(struct musb *musb)
{
	int ret = 0;

	musb->typec_nb.notifier_call = otg_tcp_notifier_call;
	ret = tusb_register_client(&musb->typec_nb);
	if (ret)
		pr_err("failed to register tusb client\n");
}
#endif
/*  vivo add end */
